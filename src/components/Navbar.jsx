...
const navItems = ['Zues', 'Voldomot', 'Elden', 'About', 'Contact']

function Navbar() {

    const [isAudioPlaying, setIsAudioPlaying] = useState(false)
    const [isIndicatioActive, setIndicatiorActive] = useState(false)
            ...
            <div className='flex h-full items-center'>
                ...
                <button className='ml-10 flex items-center space-x-0.5' onClick={toggleAudioIndicator}>
                    <audio src="/audio/loop.mp3" loop className='hidden' ref={audioElementRef} />
                        {[1, 2, 3, 4].map((bar)=>(
                            <div key={bar} className={`indicator-line ${isIndicatioActive ? 'active': ''}`} style={{animationDelay: `${bar*0.1}s`}} />
                        ))}
